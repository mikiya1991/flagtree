...
@triton.jit
def mm_kernel(a_desc_ptr, b_desc_ptr, c_desc_ptr, M, N, K, BLOCK_SIZE_M: tl.constexpr, BLOCK_SIZE_N: tl.constexpr,
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        a = tl._experimental_descriptor_load(a_desc_ptr, [offs_am, offs_k], [BLOCK_SIZE_M, BLOCK_SIZE_K], tme_load_type)
        b = tl._experimental_descriptor_load(b_desc_ptr, [offs_k, offs_bn], [BLOCK_SIZE_K, BLOCK_SIZE_N], tme_load_type)
...
num_stages_list = [1]
test_params = [(elem_type, M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, num_warps, num_stages)
...
@pytest.mark.parametrize("elem_type, M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, num_warps, num_stages", test_params)
def test_mm_kernel_perf(elem_type, M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, num_warps, num_stages):
    device = "musa"
    ab_type = elem_type
    c_type = elem_type
    ...
    ms_triton = triton.musa_testing.do_bench(lambda: mm_kernel[(triton.cdiv(M, BLOCK_M) * triton.cdiv(
        N, BLOCK_N), 1, 1)](desc_a, desc_b, desc_c, M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, get_triton_type(ab_type),
    ...
    print(f"torch mm latency: {ms_torch:.4f} ms")
    print(f"triton mm kernel latency: {ms_triton:.4f} ms")
